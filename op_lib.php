...
function add_column($table_name, $col_name, $data_type ='varchar(255)', $default =null )
	{
		global $con;
		$result = direct_sql("SHOW COLUMNS FROM `$table_name` LIKE '$col_name'")['count'];
		$res = array();
		if($result>0){
			$res['msg'] = 'Column already exists '.$col_name;
			$res['status'] = 'error';
		}else{
			$sql ="alter table $table_name add column $col_name $data_type $default"; 
		    $resp =mysqli_query($con,$sql) or die("Column already exists". mysqli_error($con));
		    $res['msg'] = $resp;
			$res['status'] = 'success';
		}
		return $res;	
...
function dz_upload_img($tbl ,$id, $target_dir = "upload", $img_for_field = 'school_id' ,$img_field ='photo'){
	// print_r($_FILES);
	  if($_FILES['file']['name'] != ''){
	  	$file_names = '';
	  	...
	  	for($i=0; $i<$total; $i++){
	  		...
	  		$valid_extension = array("png","jpg","jpeg","pdf");
	  if(in_array($extention, $valid_extension)){
	  	$new_name = $id.rand(100,999).$file_name;
	  	$path = "../$target_dir/" . $new_name;
	  	$resp = move_uploaded_file($_FILES['file']['tmp_name'][$i],$path);
	  	 if ($resp ==1){
	  	 	// $file_names .= $new_name . ",";
	  	 	$name_arr = array($img_for_field=>$id,$img_field=>$new_name);
 ...
 function dz_remove_img($target_dir = "upload",$name){
		        $path1 = "../$target_dir/";
...
function btn_delete_img($table, $id, $name, $disabled ="" , $icon = 'fa fa-trash' , $per = 'no')
		{
			$str ="<button class='delete_btn_img btn text-light btn-sm'  data-table='$table' data-id='$id' data-pkey='id' data-per='$per' data-name='$name' title='Detete This Permanently' $disabled > <i class='$icon'></i> </button> ";
			return $str ;
		}
function btn_delete_field($table,$column_table,$column_name,$id, $disabled ="" , $icon = 'fa fa-trash' , $per = 'no')
		{
			$str ="<button class='field_delete_btn btn btn-danger btn-sm'  data-table='$table' data-id='$id' data-column_table='$column_table' data-column_name='$column_name' data-pkey='id' data-per='$per' title='Detete This Permanently' $disabled > <i class='$icon'></i> </button> ";
			return $str ;
		}
